...
public class WorldBrain : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (; crickets > 0; crickets--) {
            platform = platforms[Random.Range(0, platforms.Count)].gameObject;
            crkt = Instantiate(cricket, platform.transform.position, Quaternion.identity);
            crkt.transform.parent = platform.transform;
        }
    }
